        ...
        <div className="glass-panel animate-fade-in" style={{ height: "100%", display: "flex", flexDirection: "column" }}>
                ...
                {menuItems.map((item, i) => (
                    <button
                        ...
                        style={{
                            ...
                            border: "none",
                            background: item.active ? "rgba(2, 132, 199, 0.05)" : "transparent",
            ...
            <div style={{ marginTop: "auto", paddingTop: "2rem", borderTop: "1px solid var(--border-color)", fontSize: "0.8rem", color: "var(--text-muted)" }}>
                <div className="flex-row gap-2" style={{ marginBottom: "0.5rem" }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--status-success)", boxShadow: "0 0 8px rgba(16, 185, 129, 0.4)" }}></div>
                    <span>Engine Status: Online</span>
                </div>
                <div>Processing realtime stream</div>
            </div>
        </div>
    );
}
